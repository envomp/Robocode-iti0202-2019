...
public class Agonaut extends AdvancedRobot {
  ...
  public void run() {
    ...
    fieldRectangle =
        new java.awt.geom.Rectangle2D.Double(36.0D, 36.0D, this.width - 36.0D, this.height - 36.0D);
    if (this.getOthers() > 2) {
  ...
  public void onScannedRobot(ScannedRobotEvent e) {
    if (this.enemy.none()
        || e.getDistance() < this.enemy.getRadius() - 70.0D
        || e.getName().equals(this.enemy.getName())) {
    ...
    if (this.getOthers() <= 2) {
      ...
      this.setTurnGunRightRadians(
          RobotFormulas.normalize(this.circularTargetAngle() - this.getGunHeadingRadians()));
  ...
  private void velocityChanger(AdvancedRobot r) {
    if (enemyEnergy > (enemyEnergy = this.enemy.getEnergy())) {
      this.setMaxVelocity(
          (double)
              Math.min((int) (this.getVelocity() * this.enemy.getSpeed() / (double) velocity), 8));
  ...
  private void GFTarget() {
    ...
    MovePrediction.targetLocation =
        RobotFormulas.project(movePrediction.gunLocation, this.absBearing, this.enemyDistance);
    ...
    this.setTurnGunRightRadians(
        RobotFormulas.normalize(
            this.absBearing
                ...
                + movePrediction.mostVisitedBearingOffset()));
    if (this.getGunHeat() == 0.0D) {
  ...
  private void doScanner() {
    if (this.getOthers() <= 2) {
      if (this.enemy.none()) {
        ...
        double turn =
            this.getHeadingRadians() - this.getRadarHeadingRadians() + this.enemy.getBearing();
  ...
  private double circularTargetAngle() {
    ...
    for (double time = 0.0D;
        (time + 1.0D) * RobotFormulas.bulletSpeed(firePower)
            < Point2D.distance(this.getX(), this.getY(), futureX, futureY);
        ++time) {
      ...
      if (futureX < 36.0D
          || futureY < 36.0D
          || futureX > this.width - 36.0D
          || futureY > this.height - 36.0D) {
  ...
  private void movePredicting() {
    ...
    this.setTurnRadarRightRadians(
        RobotFormulas.normalize(this.absBearing - this.getRadarHeadingRadians()) * 2.0D);
    this.surfDirections.add(0, this.lateralVelocity >= 0.0D ? 1 : -1);
    this.surfAbsBearings.add(0, this.absBearing + Math.PI);
    this.enemyLocation =
        RobotFormulas.project(this.myLocation, this.absBearing, this.enemyDistance);
    ...
    if (this.getOthers() <= 2) {
      if (!this.enemyMovePrediction.isEmpty()) {
        ...
        for (EnemyMovePrediction movePrediction : this.enemyMovePrediction) {
          if (Math.abs(
                      movePrediction.distanceTraveled
                          - this.myLocation.distance(movePrediction.fireLocation))
                  < 50.0D
              && Math.round(RobotFormulas.bulletSpeed(e.getBullet().getPower()) * 10.0D)
  ...
  private EnemyMovePrediction getClosestMovePrediction() {
    ...
    for (EnemyMovePrediction movePrediction : this.enemyMovePrediction) {
      double distance =
          this.myLocation.distance(movePrediction.fireLocation) - movePrediction.distanceTraveled;
    ...
    return (int)
        RobotFormulas.limit(
            ...
            RobotFormulas.normalize(
                    ...
                    / RobotFormulas.maxEscapeAngle(ew.bulletVelocity)
                    * (double) ew.direction
                    * (double) ((movePredictions.length - 1) / 2)
                + (double) ((movePredictions.length - 1) / 2),
  ...
  private Point2D.Double predictPosition(EnemyMovePrediction prediction, int direction) {
    ...
    do {
      ...
      predictedHeading =
          RobotFormulas.normalize(
              predictedHeading + RobotFormulas.limit(-maxTurning, moveAngle, maxTurning));
      ...
      predictedPosition =
          RobotFormulas.project(predictedPosition, predictedHeading, predictedVelocity);
      ++counter;
      if (predictedPosition.distance(prediction.fireLocation)
          < prediction.distanceTraveled
              + (double) counter * prediction.bulletVelocity
              + prediction.bulletVelocity) {
        intercepted = true;
      }
  ...
  private void dodgeAccordingToAngle(AdvancedRobot r, double goAngle) {
    double angle = RobotFormulas.normalize(goAngle - r.getHeadingRadians());
    angle = angle * 0.8 + (angle * 1.2 - angle * 0.8) * random.nextDouble();
